...
export default function Elders() {
  ...
  const handleFeatureNavigation = (route: string) => {
    // Store the selected elder ID in sessionStorage so the feature page can use it
    if (selectedProfile) {
      sessionStorage.setItem("selectedElderId", selectedProfile.id);
      sessionStorage.setItem("selectedElderName", selectedProfile.name);
    }
    navigate(route);
  };
      ...
      <div className="flex min-h-screen bg-black text-white relative overflow-hidden">
        ...
        <div className={`z-30 w-72 sm:w-80 bg-[#1e1e1e]/95 backdrop-blur-md border-r border-gray-800 flex-col shadow-2xl ${
          ...
          <div className="p-3 sm:p-4 flex items-center justify-between lg:block border-b border-gray-800">
            <Button
              type="button"
              onClick={() => navigate("/manage-profile", { replace: true })}
          ...
          <div className="p-4 sm:p-6 border-b border-gray-800 bg-[#131313]/50">
            <div className="flex justify-between items-center mb-3 sm:mb-4">
              <h2 className="text-lg sm:text-xl font-bold text-white">Elder Profiles</h2>
          ...
          {selectedProfile ? (
            <div className="space-y-6 sm:space-y-8">
              ...
              <div className="space-y-4 sm:space-y-6">
                {/* Profile Header Card */}
                <Card className="bg-[#1e1e1e] border border-gray-800 hover:shadow-xl transition-shadow duration-200 mt-0 sm:mt-0">
                      ...
                      <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0 w-full">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs sm:text-sm text-gray-400 mt-1">
                            {selectedProfile.relationship || "N/A"} • Age {selectedProfile.age} • Updated {formatDate(selectedProfile.lastUpdated)}
                          </p>
                        </div>
                      </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4">
                  {featureCards.map((feature, idx) => {
                    ...
                    return (
                      <Card
                        key={idx}
                        onClick={() => handleFeatureNavigation(feature.route)}
                      ...
                      >
                        <CardContent className="p-4 sm:p-6 flex flex-col items-center text-center space-y-3">
                          ...
                          <Button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleFeatureNavigation(feature.route);
                            }}
